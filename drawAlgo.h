...
// The drawing functions call given callback funcions for drawing pixels. The algorithms are
// based on the bresenham approach <http://members.chello.at/easyfilter/bresenham.html> and
//
...
#define daSet(col) [bmp2](int16_t x, int16_t y) { \
  bmp2->setPixel(x, y, col); \
}

// prototype to calc the color for a pixel
typedef std::function<RGBA(int16_t x, int16_t y)> fGetColor;
...
public:
  ...
  void polyFill(Segment *segments, uint16_t len, fSetPixel cbBorder, fSetPixel cbFill = nullptr) {
    std::vector<DAPoint> borderPoints;
    fSetPixel fStroke = cbBorder ? cbBorder : cbFill;  // use cbFill when no cbBorder is given.

    polyBorder(segments, len,
               [&](int16_t x, int16_t y) {
    ...
    // for (DAPoint p : borderPoints) printf("%d/%d ", p.x, p.y);
    // printf("\n\n");
    ...
    for (DAPoint &p : borderPoints) {
      ...
      if (p.y != y) {
        ...
        if ((nextX > x + 1) && (isInside)) {
          // printf("(%d/%d)-(%d/%d) ", x + 1, y, nextX - 1, y);
          // draw the line
          while (++x < nextX) {
            cbFill(x, y);
          }
  ...
  void polyBorder(Segment *segments, uint16_t len, fSetPixel cbBorder) {
    int16_t startPosX = 0;
    int16_t startPosY = 0;
    int16_t posX = 0;
    int16_t posY = 0;
    ...
    for (uint16_t n = 0; n < len; n++) {
      ...
      switch (pSeg->type) {
        ...
        case 'L':
          endPosX = pSeg->x1;
          endPosY = pSeg->y1;
          line(posX, posY, endPosX, endPosY, cbBorder);
          break;
