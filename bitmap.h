...
// clang-format off
  ...
  const RGBA SILVER = { 0xCC, 0xCC, 0xCC, 0xFF };
  const RGBA RED    = { 0xFF,    0,    0, 0xFF };
  const RGBA ORANGE = { 0xE9, 0x76,    0, 0xFF };
  const RGBA YELLOW = { 0xF6, 0xC7,    0, 0xFF };
  const RGBA GREEN  = {    0, 0xFF,    0, 0xFF };
  const RGBA BLUE   = {    0,    0, 0xFF, 0xFF };
  const RGBA CYAN   = {    0, 0xFF, 0xFF, 0xFF };
  const RGBA PURPLE = { 0x99, 0x46, 0x80, 0xFF };
...
public:
  ...
  bool Load(const char* Filename) {
    ...
    m_BitmapData = new gfxDraw::RGBA[m_BitmapSize];
  ...
  // Added to manipulate image based on pixels
  void setPixel(int32_t x, int32_t y, gfxDraw::RGBA color) {
    int w = GetWidth();
    int h = GetHeight();
    if ((x < 0) || (x > w) || (y < 0) || (y > h)) {
      printf("bad %d/%d\n", x, y);
      ...
      uint32_t Offset = (w * (h-1)) - (w * y) + x;
      m_BitmapData[Offset] = color;
    }
  }
  ...
  bool GetBits(void* Buffer, unsigned int& Size) {
    bool Result = false;
    if (Size == 0 || Buffer == 0) {
      Size = m_BitmapSize * sizeof(gfxDraw::RGBA);
  ...
  bool GetBits(void* Buffer, unsigned int& Size, unsigned int RedMask, unsigned int GreenMask, unsigned int BlueMask, unsigned int AlphaMask, bool IncludePadding = true) {
    ...
    for (unsigned int i = 0; i < m_BitmapSize; i++) {
      *(uint32_t*)BufferPtr =
        (CColor::Convert(m_BitmapData[i].Blue, 8, BitCountBlue) << BitPosBlue) | (CColor::Convert(m_BitmapData[i].Green, 8, BitCountGreen) << BitPosGreen) | (CColor::Convert(m_BitmapData[i].Red, 8, BitCountRed) << BitPosRed) | (CColor::Convert(m_BitmapData[i].Alpha, 8, BitCountAlpha) << BitPosAlpha);
